...
#define REMOTE_CALCULATE_ID 0x0001
...
void split(double weight) {
	...
	athread_attr_init(&remote_attr);
	athread_attr_set_remote_ability(&remote_attr, 1);
	athread_attr_set_remote_service(&remote_attr, REMOTE_CALCULATE_ID);
...
int main(int argc, char *argv[]) {
  ...
  MPI_Comm_size(MPI_COMM_WORLD, &athread_remote_size);
	...
	weight = atof(argv[1]);
	aInit(&argc, &argv);
	athread_remote_register_service(REMOTE_CALCULATE_ID, calculate);
